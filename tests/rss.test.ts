...
test("generateRssFeed returns RSS XML with channel metadata and article links", async () => {
  ...
  assert.match(xml, /<item>[\s\S]*<title>Warum Warteschlangen nicht schneller werden: Little&apos;s Law<\/title>/);
  assert.match(xml, /<link>https:\/\/thetheobot\.github\.io\/articles\/2026-05-07-warteschlangen-und-littles-law<\/link>/);
  assert.match(xml, /<category>Warteschlangentheorie<\/category>/);
});
